...
const static CRC32_JAMCRC crc32_jamcrc_default = {
#include CRC32_JAMCRC_DEFAULT_DATA
};

const static CRC32_JAMCRC_NUM_TYPE crc32_jamcrc_table[] = {
#include CRC32_JAMCRC_TABLE_DATA
};

static CRC32_JAMCRC_NUM_TYPE crc32_jamcrc_reverse_bits(CRC32_JAMCRC_NUM_TYPE data, uint8_t width) {
    CRC32_JAMCRC_NUM_TYPE result = 0;
...
void crc32_jamcrc_init(CRC32_JAMCRC *crc) {
    memcpy(crc, &crc32_jamcrc_default, sizeof(CRC32_JAMCRC));
}

static CRC32_JAMCRC_NUM_TYPE crc32_jamcrc_calc_reflect(CRC32_JAMCRC *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC32_JAMCRC_NUM_TYPE crc_val;
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        ...
        crc_val ^= crc32_jamcrc_table[index];
    }
...
CRC32_JAMCRC_NUM_TYPE crc32_jamcrc_calc(CRC32_JAMCRC *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC32_JAMCRC_NUM_TYPE crc_val, data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc32_jamcrc_calc_reflect(crc, data, length);
    }
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        if (crc->input_reflected) {
            data_xor = crc32_jamcrc_reverse_bits(*value, 8) & 0xFF;
        } else {
            data_xor = *value;
        }
    ...
    if (crc->result_reflected) {
        crc_val = crc32_jamcrc_reverse_bits(crc_val, crc->width);
    }

    return crc_val ^ crc->final_xor_value;
}

static CRC32_JAMCRC_NUM_TYPE crc32_jamcrc_accum_reflect(CRC32_JAMCRC *crc, void *data, size_t length) {
    uint8_t *value, index;
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        ...
        crc->accumulate ^= crc32_jamcrc_table[index];
    }
...
CRC32_JAMCRC_NUM_TYPE crc32_jamcrc_accum(CRC32_JAMCRC *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC32_JAMCRC_NUM_TYPE data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc32_jamcrc_accum_reflect(crc, data, length);
    }
    ...
    if (crc->result_reflected) {
        crc->accumulate = crc32_jamcrc_reverse_bits(crc->accumulate, crc->width);
    }
