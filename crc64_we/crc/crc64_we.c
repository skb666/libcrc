...
const static CRC64_WE crc64_we_default = {
#include CRC64_WE_DEFAULT_DATA
};

const static CRC64_WE_NUM_TYPE crc64_we_table[] = {
#include CRC64_WE_TABLE_DATA
};

static CRC64_WE_NUM_TYPE crc64_we_reverse_bits(CRC64_WE_NUM_TYPE data, uint8_t width) {
    CRC64_WE_NUM_TYPE result = 0;
...
void crc64_we_init(CRC64_WE *crc) {
    memcpy(crc, &crc64_we_default, sizeof(CRC64_WE));
}

static CRC64_WE_NUM_TYPE crc64_we_calc_reflect(CRC64_WE *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC64_WE_NUM_TYPE crc_val;
...
CRC64_WE_NUM_TYPE crc64_we_calc(CRC64_WE *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC64_WE_NUM_TYPE crc_val, data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc64_we_calc_reflect(crc, data, length);
    }
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        if (crc->input_reflected) {
            data_xor = crc64_we_reverse_bits(*value, 8) & 0xFF;
        } else {
            data_xor = *value;
        }
    ...
    if (crc->result_reflected) {
        crc_val = crc64_we_reverse_bits(crc_val, crc->width);
    }

    return crc_val ^ crc->final_xor_value;
}

static CRC64_WE_NUM_TYPE crc64_we_accum_reflect(CRC64_WE *crc, void *data, size_t length) {
    uint8_t *value, index;
...
CRC64_WE_NUM_TYPE crc64_we_accum(CRC64_WE *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC64_WE_NUM_TYPE data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc64_we_accum_reflect(crc, data, length);
    }
    ...
    if (crc->result_reflected) {
        crc->accumulate = crc64_we_reverse_bits(crc->accumulate, crc->width);
    }
