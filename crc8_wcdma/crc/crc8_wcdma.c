...
const static CRC8_WCDMA crc8_wcdma_default = {
#include CRC8_WCDMA_DEFAULT_DATA
};

const static CRC8_WCDMA_NUM_TYPE crc8_wcdma_table[] = {
#include CRC8_WCDMA_TABLE_DATA
};

static CRC8_WCDMA_NUM_TYPE crc8_wcdma_reverse_bits(CRC8_WCDMA_NUM_TYPE data, uint8_t width) {
    CRC8_WCDMA_NUM_TYPE result = 0;
...
void crc8_wcdma_init(CRC8_WCDMA *crc) {
    memcpy(crc, &crc8_wcdma_default, sizeof(CRC8_WCDMA));
}

static CRC8_WCDMA_NUM_TYPE crc8_wcdma_calc_reflect(CRC8_WCDMA *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC8_WCDMA_NUM_TYPE crc_val;
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        ...
        crc_val ^= crc8_wcdma_table[index];
    }
...
CRC8_WCDMA_NUM_TYPE crc8_wcdma_calc(CRC8_WCDMA *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC8_WCDMA_NUM_TYPE crc_val, data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc8_wcdma_calc_reflect(crc, data, length);
    }
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        if (crc->input_reflected) {
            data_xor = crc8_wcdma_reverse_bits(*value, 8) & 0xFF;
        } else {
            data_xor = *value;
        }
    ...
    if (crc->result_reflected) {
        crc_val = crc8_wcdma_reverse_bits(crc_val, crc->width);
    }

    return crc_val ^ crc->final_xor_value;
}

static CRC8_WCDMA_NUM_TYPE crc8_wcdma_accum_reflect(CRC8_WCDMA *crc, void *data, size_t length) {
    uint8_t *value, index;
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        ...
        crc->accumulate ^= crc8_wcdma_table[index];
    }
...
CRC8_WCDMA_NUM_TYPE crc8_wcdma_accum(CRC8_WCDMA *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC8_WCDMA_NUM_TYPE data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc8_wcdma_accum_reflect(crc, data, length);
    }
    ...
    if (crc->result_reflected) {
        crc->accumulate = crc8_wcdma_reverse_bits(crc->accumulate, crc->width);
    }
