...
const static CRC16_MAXIM crc16_maxim_default = {
#include CRC16_MAXIM_DEFAULT_DATA
};

const static CRC16_MAXIM_NUM_TYPE crc16_maxim_table[] = {
#include CRC16_MAXIM_TABLE_DATA
};

static CRC16_MAXIM_NUM_TYPE crc16_maxim_reverse_bits(CRC16_MAXIM_NUM_TYPE data, uint8_t width) {
    CRC16_MAXIM_NUM_TYPE result = 0;
...
void crc16_maxim_init(CRC16_MAXIM *crc) {
    memcpy(crc, &crc16_maxim_default, sizeof(CRC16_MAXIM));
}

static CRC16_MAXIM_NUM_TYPE crc16_maxim_calc_reflect(CRC16_MAXIM *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC16_MAXIM_NUM_TYPE crc_val;
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        ...
        crc_val ^= crc16_maxim_table[index];
    }
...
CRC16_MAXIM_NUM_TYPE crc16_maxim_calc(CRC16_MAXIM *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC16_MAXIM_NUM_TYPE crc_val, data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc16_maxim_calc_reflect(crc, data, length);
    }
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        if (crc->input_reflected) {
            data_xor = crc16_maxim_reverse_bits(*value, 8) & 0xFF;
        } else {
            data_xor = *value;
        }
    ...
    if (crc->result_reflected) {
        crc_val = crc16_maxim_reverse_bits(crc_val, crc->width);
    }

    return crc_val ^ crc->final_xor_value;
}

static CRC16_MAXIM_NUM_TYPE crc16_maxim_accum_reflect(CRC16_MAXIM *crc, void *data, size_t length) {
    uint8_t *value, index;
    ...
    for (size_t i = 0; i < length; ++value, ++i) {
        ...
        crc->accumulate ^= crc16_maxim_table[index];
    }
...
CRC16_MAXIM_NUM_TYPE crc16_maxim_accum(CRC16_MAXIM *crc, void *data, size_t length) {
    uint8_t *value, index;
    CRC16_MAXIM_NUM_TYPE data_xor;

    if (crc->input_reflected && crc->result_reflected) {
        return crc16_maxim_accum_reflect(crc, data, length);
    }
    ...
    if (crc->result_reflected) {
        crc->accumulate = crc16_maxim_reverse_bits(crc->accumulate, crc->width);
    }
